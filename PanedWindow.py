...
class panedWindow(tk.Frame):
    def __init__(self, root ,master=None ):
        ...
        # Frameの作成
        widget1 = imgFrame.frameGUI(root)
        widget1.popUpMenu.add_command(label='ウィンドウ分割',underline=5,command=lambda:self.create_paned_frame(paned_window, root))
        widget1.update()
    ...
    def create_paned_frame(self, parentWindow, root):
        # PanedWindowの作成
        paned_window2 = tk.PanedWindow(parentWindow, sashwidth = 4)
        paned_window2.pack(expand = True, fill = tk.BOTH)
        
        # Frameの作成
        widget2 = imgFrame.frameGUI(root)
        widget2.popUpMenu.add_command(label='ウィンドウ分割',underline=5,command=lambda:self.create_paned_frame(paned_window2, root))
        widget2.update()
        
        # フレームをPanedWindowに追加
        paned_window2.add(parentWindow.panes()[0]) # クリック元のFrame
